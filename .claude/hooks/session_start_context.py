...
SECURITY_RULES = [
    ...
    (r"\b(export\s+.*PASSWORD|export\s+.*TOKEN)", "Exposed credentials in command detected"),
]

# Prompt validation rules
PROMPT_VALIDATION_RULES = [
    (r"\bgrep\b(?!.*\|)", "Consider using 'rg' (ripgrep) instead of 'grep' for better performance"),
    (r"\bfind\s+.*-name", "Consider using 'rg --files -g pattern' instead of 'find -name' for better performance"),
]
...
def get_project_metadata() -> str:
    ...
    if os.path.exists("pubspec.yaml"):
        ...
        try:
            with open("pubspec.yaml", 'r') as f:
                ...
                deps = re.findall(r'^\s+([a-zA-Z][a-zA-Z0-9_]*):.*$', content, re.MULTILINE)
                if deps:
                    key_deps = [dep for dep in deps[:5] if dep not in ['flutter', 'cupertino_icons']]
                    if key_deps:
        ...
        try:
            with open("requirements.txt", 'r') as f:
                deps = [line.split('==')[0].split('>=')[0].strip() 
                       for line in f.readlines()[:5] if line.strip() and not line.startswith('#')]
                if deps:
                    metadata.append(f"Key Dependencies: {', '.join(deps)}")
        except Exception:
            pass
    ...
    try:
        ...
        for ext in extensions:
            try:
                result = subprocess.run(
                    ["find", ".", "-name", f"*{ext}", "-type", "f", "!", "-path", "./node_modules/*", 
                     "!", "-path", "./.git/*", "!", "-path", "./build/*", "-printf", "%T@ %p\n"],
                    capture_output=True, text=True, timeout=10
                )
                if result.returncode == 0:
                    files = [(float(line.split()[0]), line.split()[1]) 
