"""RAG-enabled chat orchestrator with document retrieval and context management.

Combines LangChain's chat orchestration with LlamaIndex retrieval and context
window management to provide RAG (Retrieval-Augmented Generation) capabilities.
"""
...
from app.schemas.chat import ChatRequest, ChatResponse, RagChatResponse, SourceCitation
...
class RagOrchestrator:
    """RAG-enabled chat orchestrator with document retrieval.

    Combines LangChain-based chat orchestration with LlamaIndex retrieval
    and context window management for RAG capabilities. Reuses the same
    session management pattern as LangChainOrchestrator.
    """
...
If the context doesn't contain relevant information, say so clearly."""
    ...
    def __init__(
        ...
        session_store: Optional[Dict[str, BaseChatMessageHistory]] = None,
    ):
        """Initialize the RAG orchestrator.

        Args:
            llm_service: LLM service for generating responses
            retriever: LlamaIndex-backed retriever for document retrieval
            ...
            session_store: Optional shared session store with LangChainOrchestrator
        """
        ...
        # Session store - use provided one or create new
        ...
        logger.info(
            "RagOrchestrator initialized with retriever and context manager"
        )
    ...
    def get_session_history(self, session_id: str) -> BaseChatMessageHistory:
        ...
        Creates a new session if it doesn't exist. This method is synchronous
        for compatibility with LangChain's RunnableWithMessageHistory.
        ...
        if session_id not in self._session_store:
            try:
                ...
                logger.info("New RAG chat session: %s", session_id)
            except Exception as e:
    ...
    async def clear_session(self, session_id: str) -> bool:
        ...
        if session_id in self._session_store:
            ...
            logger.info("Cleared RAG chat session: %s", session_id)
            return True
        return False
    ...
    async def clear_all_sessions(self) -> int:
        ...
        logger.info("Cleared %d RAG chat sessions", session_count)
    ...
    async def health_check(self) -> bool:
        """Check if the RAG orchestrator is healthy.
        ...
        try:
            ...
            if not llm_service_healthy:
                logger.warning("RAG orchestrator health check: LLM service unhealthy")
                return False
            ...
            try:
                ...
                logger.warning(
                    "RAG orchestrator health check: Retriever error: %s", str(e)
                )
                return False

            session_count = len(self._session_store)
            logger.debug(
                "RAG orchestrator health: LLM OK, retriever OK, %d active sessions",
                session_count,
            )
            ...
            logger.error("RAG orchestrator health check failed: %s", str(e))
            return False
