...
Repositories are provided through the DI container (`app.di.providers.repositories`). Use `Provide[FitviseContainer.repositories.*]` in endpoints and the container’s test overrides for unit tests. Transactional sessions are available via `container.repositories.transaction_session()`. No legacy containers required.
