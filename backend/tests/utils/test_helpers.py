...
class AssertionHelpers:
    ...
    def assert_valid_workout_response(response_data: Dict[str, Any]):
        ...
        required_fields = ["response", "tokens_used", "response_time", "model", "success"]
    ...
    def assert_valid_health_response(response_data: Dict[str, Any]):
        ...
        assert response_data["status"] in ["healthy", "unhealthy"], "Status must be 'healthy' or 'unhealthy'"
        assert isinstance(response_data["model"], str), "Model must be a string"
        assert isinstance(response_data["response_time"], (int, float)), "Response time must be numeric"
    ...
    def assert_api_error_response(response_data: Dict[str, Any], expected_status: int = None):
        ...
        if expected_status:
            assert response_data["status_code"] == expected_status, f"Expected status {expected_status}, got {response_data['status_code']}"
...
class TestEnvironment:
    ...
    @staticmethod
    def setup_test_env_vars(env_overrides: Dict[str, str] = None) -> Dict[str, Optional[str]]:
        """
