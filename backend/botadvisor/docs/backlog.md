...
| C1 | Retriever contract & registry | `partial` | `backend/botadvisor/app/core/entity/retriever_request.py`, `backend/botadvisor/app/core/entity/retriever_result.py` | Shared DTOs exist, but canonical retrieval contract and registry are not assembled |
| C2 | LlamaIndex retriever | `partial` | `backend/app/infrastructure/llm/services/llama_index_retriever.py` | A reusable legacy retriever exists, but it is not migrated into `botadvisor` and lacks canonical hybrid-search packaging |
| C3 | LangChain adapter tooling | `partial` | `backend/app/infrastructure/llm/services/llama_index_retriever.py`, `backend/app/api/v1/fitvise/deps.py` | LangChain compatibility exists in legacy code, but not as canonical `botadvisor` tooling |
...
| D3 | FastAPI surface | `partial` | `backend/app/api/v1/fitvise/chat.py` | Chat and health endpoints exist in legacy runtime, not in canonical `botadvisor` |
