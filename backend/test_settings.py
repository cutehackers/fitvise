...
def validate_configuration_fields(settings) -> Dict[str, Any]:
    ...
    field_validations = {
        ...
        'environment': {'type': str, 'required': True, 'values': ['local', 'staging', 'production']},
        ...
        'vector_store_type': {'type': str, 'required': True, 'values': ['chromadb', 'faiss']},
    ...
    for field_name, validation in field_validations.items():
        try:
            ...
            if not isinstance(value, validation['type']):
                validation_results['failed'] += 1
                validation_results['errors'].append(f"{field_name}: Wrong type {type(value)}, expected {validation['type']}")
            ...
            if 'values' in validation and value not in validation['values']:
                validation_results['failed'] += 1
                validation_results['errors'].append(f"{field_name}: Invalid value '{value}', must be one of {validation['values']}")
            ...
            if 'min_length' in validation and isinstance(value, str) and len(value) < validation['min_length']:
                validation_results['failed'] += 1
                validation_results['errors'].append(f"{field_name}: String too short, minimum {validation['min_length']} characters")
